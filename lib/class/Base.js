...
class Base {
	...
	constructor() {
		this.mch_id = config.getConfig('mch_id'); // 企业ID
		this.appid = config.getConfig('appid'); // 应用ID
		this.serial_no = config.getConfig('serial_no'); // 序列号
		this.api_v3_cert = fs.readFileSync(path.join(process.cwd(),'/',config.getConfig('api_v3_cert_path'))); // 证书
		this.private_key = fs.readFileSync(path.join(process.cwd(),'/',config.getConfig('private_key_path')));  // 私钥
		this.public_key = fs.readFileSync(path.join(process.cwd(),'/',config.getConfig('public_key_path')));  // 公钥
		this.public_key_id = config.getConfig('public_key_id'); // 公钥ID
		console.log(this)
	}
