...
const BASE_URL_BAK = "https://api2.mch.weixin.qq.com"
//#endregion
...
class Base {
	...
	constructor(data) {
		...
		this.api_v3_key = api_v3_key || config.getConfig('api_v3_key'); // APIv3密钥
	}
	 ...
	 * @param {string} params.ciphertext - 加密数据
	...
	decrypt({
		...
		const key = this.api_v3_key; // 32字节APIv3密钥，需在类实例化时传入this.api_v3_key
		if (!key) throw new Error('APIv3密钥未配置');
		if (key.length !== 32) throw new Error('APIv3密钥长度必须为32字节');
		const keyBuf = Buffer.from(key, 'utf8');
		const nonceBuf = Buffer.from(nonce, 'utf8');
		const adBuf = Buffer.from(associated_data, 'utf8');
		const data = Buffer.from(ciphertext, 'base64');
		// AES-256-GCM解密
		const decipher = crypto.createDecipheriv('aes-256-gcm', keyBuf, nonceBuf);
		decipher.setAuthTag(data.slice(data.length - 16));
		decipher.setAAD(adBuf);
	 ...
	 */
	async curl(_url, _method, _params, _version = 'v3', _use_bak = false) {
		...
		return new Promise((resolve, reject) => {
			...
			const url = _use_bak ? BASE_URL_BAK + _url : BASE_URL + _url;
